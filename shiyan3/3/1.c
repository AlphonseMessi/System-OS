...
int main(void)
{
    ...
    id = msgget(IPC_PRIVATE, 0666|IPC_CREAT);	// 创建消息队列
    if (msgid < 0) {
        printf("[*] Error: msgget() return error\n");
        exit(1);
    }
    pthread_create(&send_pid_1, NULL, sender1, NULL); // 创建 线程，并将线程加入当前 进程
    pthread_create(&send_pid_2, NULL, sender2, NULL);
    pthread_create(&recv_pid, NULL, receiver, NULL);
